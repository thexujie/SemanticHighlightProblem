...
namespace XE
{
	FFile::FFile(FStringView FileName)
	{
		FWStringView WPath = Strings::ToWide(FileName);
		HANDLE hFile = CreateFileW(WPath.Data, 0, FILE_SHARE_READ | FILE_SHARE_WRITE | FILE_SHARE_DELETE, NULL, OPEN_EXISTING, 0, NULL);
	...
	FString FFile::ReadToString(FStringView FileName, EFileBOM * FileBOM)
	{
		FWStringView WPath = Strings::ToWide(FileName);
		HANDLE hFile = CreateFileW(WPath.Data, GENERIC_READ, FILE_SHARE_READ | FILE_SHARE_WRITE | FILE_SHARE_DELETE, NULL, OPEN_EXISTING, 0, NULL);
		if (hFile == INVALID_HANDLE_VALUE)
			return FString();
	...
	bool FFile::SaveToFile(FStringView FileName, FStringView String)
	{
		FWStringView WPath = Strings::ToWide(FileName);
		HANDLE hFile = CreateFileW(WPath.Data, GENERIC_WRITE, FILE_SHARE_READ, NULL, CREATE_ALWAYS, 0, NULL);
		if (hFile == INVALID_HANDLE_VALUE)
			return false;

		DWORD NumberOfBytesWriten = 0;
		BOOL bReadFile = WriteFile(hFile, String.GetData(), DWORD(String.GetSize()) * sizeof(Char), &NumberOfBytesWriten, NULL);
		CloseHandle(hFile);
		
		return NumberOfBytesWriten == String.GetSize() * sizeof(Char);
	}

	TVector<byte_t> FFile::ReadToBytes(FStringView FileName)
	{
		FWStringView WPath = Strings::ToWide(FileName);
		HANDLE hFile = CreateFileW(WPath.Data, GENERIC_READ, FILE_SHARE_READ | FILE_SHARE_WRITE | FILE_SHARE_DELETE, NULL, OPEN_EXISTING, 0, NULL);
		if (hFile == INVALID_HANDLE_VALUE)
