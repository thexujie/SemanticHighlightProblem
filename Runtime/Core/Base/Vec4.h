...
namespace XE
{
    ...
    public:
        ...
        ~TVec4() = default;
        auto operator <=>(const TVec4 & Another) const = default; //{ return Compare(X, Another.X, Y, Another.Y, Z, Another.Z, W, Another.W); }
        ...
        //--------------------------------------------------比较运算
        template<typename = EnableIfT<IsSameV<T, decltype(std::declval<T>() + std::declval<T>())>>>
