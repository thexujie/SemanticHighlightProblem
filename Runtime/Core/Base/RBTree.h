...
namespace XE
{
	...
	public:
		...
		TPair<Iterator, bool> InsertOrAssign(const TPair<KeyT, ValueT> & Item)
		{
			...
			while (CurrentNode)
			{
				if (Comparer(Item.First, CurrentNode->Item.First))
				{
					ParentNode = CurrentNode;
					CurrentNode = CurrentNode->Left;
					IsLeft = true;
				}
				else if (Comparer(CurrentNode->Item.First, Item.First))
				{
					ParentNode = CurrentNode;
					CurrentNode = CurrentNode->Right;
					IsLeft = false;
				...
				{
					CurrentNode->Item.Second = Item.Second;
					return TPair<Iterator, bool>(Iterator(CurrentNode), false);
				}
			}
		...
		TPair<Iterator, bool> Access(const KeyT & Key)
		{
			...
			while (CurrentNode)
			{
				if (Comparer(Key, CurrentNode->Item.First))
				{
					ParentNode = CurrentNode;
					CurrentNode = CurrentNode->Left;
					IsLeft = true;
				}
				else if (Comparer(CurrentNode->Item.First, Key))
				{
					ParentNode = CurrentNode;
					CurrentNode = CurrentNode->Right;
					IsLeft = false;
