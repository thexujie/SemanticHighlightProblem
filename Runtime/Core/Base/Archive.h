...
namespace XE
{
	...
	using EArchiveFlags = TFlag<EArchiveFlag>;
	
	class CORE_API FArchive : public FStream
	{
	public:
		FArchive(FDataStream & DataStreamIn, EArchiveFlags ArchiveFlagsIn = EArchiveFlag::None) : DataStream(DataStreamIn), ArchiveFlags(ArchiveFlagsIn) {}
		~FArchive() = default;
		...
		bool IsInput() const { return ArchiveFlags.Any(EArchiveFlag::Input); }
		
	protected:
		FDataStream & DataStream;
		EArchiveFlags ArchiveFlags = EArchiveFlag::None;
	};
}
