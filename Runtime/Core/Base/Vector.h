...
namespace XE
{
	...
	public:
		...
		explicit TVector(TVectorView<T> Array)
		{
			Size = Array.Size;
			Capacity = Array.Size;
			Data = Capacity > 0 ? AllocatorT::Alloc(Capacity) : nullptr;
		...
		T * Add(TVectorView<T> Another, EInitializeMode InitializeMode = EInitializeMode::Default)
		{
			size_t Index = Size;
		...
		TVector & operator=(TVectorView<T> That)
		{
			AllocatorT::Destruct(Data, Size);
			AllocatorT::Free(Data);
	...
	template<typename T>
	struct THash<TVectorView<T>>
	{
		uintx_t operator()(const TVectorView<T> & VectorView) const noexcept
		{
