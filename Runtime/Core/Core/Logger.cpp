...
namespace XE
{
    const char * LogLevelText[] = { "Dbg", "Inf", "War", "Err" };
 ...
 //   }
    ...
    EError FLogger::Open(FStringView FileName)
    {
        ...
        FileStream.Open(FileName, EFileMode::Out);
    ...
    EError FLogger::LogToBuffer(uint32_t ProcessId, uint32_t ThreadId, ELogLevel LogLevelThis, FStringView Category, FStringView String)
    {
        ...
        {
			uint64_t TimeInMs = FDateTime::SystemNano() / 1000 / 1000;
            {
    ...
    {
        EError err = LogToBuffer(-1, -1, Level, Category, String);
        ++Line;
        return err;
    }
            ...
            for (auto & LogItem : LogItemsWriting)
            {
                ...
                else
                {
                    ...
                    int32_t nchars = _snprintf_s(temp, std::size(temp), "[%04d-%02d-%02d %02d:%02d:%02d %03d][PID:%-6d][TID:%-6d][%s][%s] ",
                        ...
                        LogItem.Category.Data,
                        LogLevelText[std::clamp((int32_t)LogItem.LogLevel, 0, (int32_t)std::size(LogLevelText))]);
