...
namespace XE
{
	...
	bool FXmlDocument::LoadFromFile(FStringView FileName)
	{
		if (!RootNode.Document)
			RootNode.Document = new rapidxml::xml_document<Char>();
		else
		...
		FString FileTextContent = FFile::ReadToString(FileName, &FileBOM);
			...
			if (EncodingStringIndex != NullIndex)
			{
				...
				if (EncodingStringEndIndex != NullIndex)
				{
					...
					else if constexpr (IsSameV<Char, char8_t>)
					{
						...
						{
							FileTextContent = Strings::AnsiToUtf8(FStringView(reinterpret_cast<const char *>(FileTextContent.Data), FileTextContent.Size));
						}
					}
				}
			}
	...
	bool FXmlDocument::SaveToFile(FStringView FileName)
	{
		std::wstring text;
		...
		FFile::SaveToFile(FileName, FileTextContent);
		return true;
	}
	...
	FString FXmlAttributes::GetString(FStringView XmlPath, FStringView NodePath, FStringView AttributeName)
	{
		...
		TVector<FStringView> Names = Split(NodePath, { Str("/\\") });
