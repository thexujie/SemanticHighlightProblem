...
int _tmain(int argc, const char *args[])
{
	...
	FCoreApplication CoreApplication;

	{
		int value = TypeOf<int>().Format(StrView("234")).Get<int>();
		Assert(value == 234);
	...
	LogInfo(Str(__FUNCTION__ " Enter Main"));
	...
	{
		TVector<FString> Entries = FDirectory::GetEntries(Path);
		PathsAll.Add(Entries);
	...
	{
		String = u8R"(CoreTest.exe”(Win32): 已加载“C:\Windows\System32\KernelBase.dll”。包含/排除设置已禁用符号加载)";
		String = R"(CoreTest.exe”(Win32): 已加载“C:\Windows\System32\KernelBase.dll”。包含/排除设置已禁用符号加载)";
		...
		FArchive SerializerOutput(DataStream, EArchiveFlag::Output);
		SerializerOutput >> ST;
	...
	MemoryStream.Reset();
	{
		...
		FDataStream DataStream(MemoryStream);
		FArchive SerializerInput(DataStream, EArchiveFlag::Input);
		SerializerInput >> ST;
	...
	StrMap.Find(Str("Key01"))->Second = Str("3123");
	...
	StrHashMap.Find(Str("Key01"))->Second = Str("3123");
	...
	for (auto StrIter = Iters.First; StrIter != Iters.Second; ++StrIter)
	{
		Strings.Add(StrIter->Second);
	}
